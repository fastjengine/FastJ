...
public class GameScene extends Scene {
  ...
  public void load(Display display) {
    ...
    playerHealthBar
        .addBehavior(playerHealthBarScript, this)
    ...
    player = createPlayer();
    player
        .addBehavior(playerControllerScript, this)
    ...
    inputManager.addKeyboardActionListener(
  ...
  private Text2D createPlayerMetaData() {
    return Text2D.create("Health: 100")
        ...
        .withTransform(
            new Pointf(27.5f, 40f), Transform2D.DefaultRotation, Transform2D.DefaultScale)
        .build();
  }
  ...
  private Polygon2D createPlayerHealthBar() {
    ...
    Pointf[] playerHealthBarMesh =
        DrawUtil.createBox(playerHealthBarMeshLocation, playerHealthBarMeshSize);

    return Polygon2D.create(playerHealthBarMesh).withFill(Color.green).build();
  }

  private Model2D createPlayer() {
    return Model2D.fromPolygons(
        ModelUtil.loadModel(Path.of(FilePaths.PathToResources + "player.psdf")));
  }
  ...
  private Model2D createEnemy() {
    ...
    Model2D enemy =
        Model2D.fromPolygons(
            ModelUtil.loadModel(Path.of(FilePaths.PathToResources + "enemy.psdf")));
