...
class Model2DTests {
  ...
  void checkModel2DCreation_withPolygon2DArrayParam() {
    ...
    Polygon2D[] polygons = {Polygon2D.fromPoints(square1), Polygon2D.fromPoints(square2)};
    ...
    assertArrayEquals(
        polygons,
        model2D.getPolygons(),
        "The created model's Polygon2D array should match the original Polygon2D array.");
    assertEquals(
        Drawable.DefaultShouldRender,
        model2D.shouldRender(),
        "The created model's 'shouldRender' option should match the default shouldRender option.");
    assertEquals(
        Transform2D.DefaultTranslation,
        model2D.getTranslation(),
        "The created model's translation should match the default translation.");
    assertEquals(
        Transform2D.DefaultRotation,
        model2D.getRotation(),
        "The created model's rotation should match the default rotation.");
    assertEquals(
        Transform2D.DefaultScale,
        model2D.getScale(),
        "The created model's scaling should match the default scale.");
  }
  ...
  @Test
  void
      checkModel2DCreation_withPolygon2DArrayParam_andRandomlyGeneratedShowParam_andRandomlyGeneratedTransformParams() {
    Pointf[] square1 = DrawUtil.createBox(Pointf.Origin, 50f);
    Pointf[] square2 = DrawUtil.createBox(Pointf.add(Pointf.Origin, 25f), 50f);
    ...
    Model2D model2D =
        Model2D.create(polygons, shouldRender)
    ...
    assertEquals(
        shouldRender,
        model2D.shouldRender(),
        "The created model's 'shouldRender' option should match the random shouldRender option.");
    assertEquals(
        randomTranslation,
        model2D.getTranslation(),
        "The created model's translation should match the random translation.");
    assertEquals(
        randomRotation,
        model2D.getRotation(),
        "The created model's rotation should match the random rotation.");
    assertEquals(
        expectedNormalizedRotation,
        model2D.getRotationWithin360(),
        "The created model's normalized rotation should match the normalized rotation.");
    assertEquals(
        randomScale,
        model2D.getScale(),
        "The created model's scaling should match the random scale.");
  }

  @Test
  void
      checkModel2DCreation_withPolygon2DArrayParam_andRandomlyGeneratedShowParam_andRandomlyGeneratedTransformParams_usingMethodChaining() {
    Pointf[] square1 = DrawUtil.createBox(Pointf.Origin, 50f);
    Pointf[] square2 = DrawUtil.createBox(Pointf.add(Pointf.Origin, 25f), 50f);
    ...
    assertEquals(
        shouldRender,
        model2D.shouldRender(),
        "The created model's 'shouldRender' option should match the expected shouldRender option.");
    assertEquals(
        randomTranslation,
        model2D.getTranslation(),
        "The created model's translation should match the expected translation.");
    assertEquals(
        randomRotation,
        model2D.getRotation(),
        "The created model's rotation should match the expected rotation.");
    assertEquals(
        expectedNormalizedRotation,
        model2D.getRotationWithin360(),
        "The created model's normalized rotation should match the normalized rotation.");
    assertEquals(
        randomScale,
        model2D.getScale(),
        "The created model's scaling should match the expected scale.");
  }
  ...
  void checkModel2DBoundsCreation_shouldMatchExpected() {
    ...
    assertArrayEquals(
        expectedBounds,
        actualBounds,
        "The actual bounds generated by the Model2D should match the expected bounds.");
  }
  ...
  void checkModel2DTranslation_shouldMatchExpected() {
    ...
    Polygon2D[] expectedPolygons = {Polygon2D.fromPoints(square1), Polygon2D.fromPoints(square2)};
    for (Polygon2D polygon2D : expectedPolygons) {
      polygon2D.translate(randomTranslation);
    }

    Polygon2D[] actualPolygons = {Polygon2D.fromPoints(square1), Polygon2D.fromPoints(square2)};
    ...
    for (Polygon2D polygon2D : expectedPolygons) {
      assertEquals(
          ...
          model2D.getTranslation(),
          "The translation of the Model2D should equal to the translation of the Polygon2Ds.");
      assertEquals(
          polygon2D.getTransformation(),
          model2D.getTransformation(),
          "The transformation of the Model2D should equal to the transformation of the Polygon2Ds.");
    }
  }
  ...
  void checkModel2DRotation_aroundOrigin_shouldMatchExpected() {
    ...
    for (Polygon2D polygon2D : expectedPolygons) {
      assertEquals(
          ...
          model2D.getRotation(),
          "The rotation of the Model2D should equal to the rotation of the Polygon2Ds.");
      assertEquals(
          polygon2D.getTransformation(),
          model2D.getTransformation(),
          "The transformation of the Model2D should equal to the transformation of the Polygon2Ds.");
  ...
  void checkModel2DScaling_aroundOrigin_shouldMatchExpected() {
    ...
    for (Polygon2D polygon2D : expectedPolygons) {
      assertEquals(
          polygon2D.getScale(),
          model2D.getScale(),
          "The scale of the Model2D should equal to the scale of the Polygon2Ds.");
      assertEquals(
          polygon2D.getTransformation(),
          model2D.getTransformation(),
          "The transformation of the Model2D should equal to the transformation of the Polygon2Ds.");
