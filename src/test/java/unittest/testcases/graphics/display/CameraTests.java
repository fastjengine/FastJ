...
class CameraTests {
  ...
  void createCamera_withTranslationPointf_andRotationFloat_andScalePointf_shouldMatchExpected() {
    ...
    assertEquals(
        ...
        "The created camera's translation should match the expected translation.");
    assertEquals(
        randomRotation,
        camera.getRotation(),
        "The created camera's rotation should match the expected rotation.");
    assertEquals(
        expectedNormalizedRotation,
        camera.getRotationWithin360(),
        "The created camera's normalized rotation should match the normalized rotation.");
  }
  ...
  void checkCameraTranslation_shouldMatchExpected() {
    ...
    Pointf expectedTranslation =
        Pointf.add(randomTranslation1, randomTranslation2).add(randomTranslation3);

    AffineTransform expectedTransform = new AffineTransform();
    expectedTransform.translate(
        randomTranslation1.x + randomTranslation2.x + randomTranslation3.x,
        randomTranslation1.y + randomTranslation2.y + randomTranslation3.y);

    assertEquals(
        expectedTranslation,
        camera.getTranslation(),
        "After translating the camera 3 times, the camera's translation should match the expected translation.");
  }
  ...
  void checkCameraRotation_shouldMatchExpected() {
    ...
    assertEquals(
        expectedRotation,
        camera.getRotation(),
        "After rotating the camera 3 times, the camera's rotation should match the expected rotation.");
    assertEquals(
        expectedNormalizedRotation,
        camera.getRotationWithin360(),
        "The created camera's normalized rotation should match the normalized rotation.");
  ...
  void checkCameraScale_shouldMatchExpected() {
    ...
    Pointf expectedScale =
        Pointf.add(randomScale1, randomScale2).add(randomScale3).add(Transform2D.DefaultScale);

    AffineTransform expectedTransform = new AffineTransform();
    expectedTransform.scale(
        randomScale1.x + randomScale2.x + randomScale3.x,
        randomScale1.y + randomScale2.y + randomScale3.y);

    assertEquals(
        expectedScale,
        camera.getScale(),
        "After scaling the camera 3 times, the camera's scale should match the expected scale.");
  }
}
