...
/**
 * Based on the {@link KeyEvent} class, the {@code Keys} class defines better-looking names for
...
public enum Keys {
  ...
  /**
   * Attempts to find a key based on the provided {@code keyName}, {@code keyCode}, and {@code
   * keyLocation}.
   *
   * <p>This method is meant for use in situations when trying to find a key while only knowing its
   * key code and location (such as from a {@link KeyEvent} -- see {@link #get(KeyEvent)}).
   *
   * <p>If possible, store the results of this computation because it searches the entire enum of
   * keys to find the correct key -- this may take longer than expected on slower machines.
   *
   ...
   * @return A {@link Keys} enum instance, if one is found. If none is found, a {@link
  ...
  public static Keys get(String keyName, int keyCode, int keyLocation) {
    return Arrays.stream(Keys.values())
        .filter(
            keys ->
                keys.name().equalsIgnoreCase(keyName)
                    || keys.name().equalsIgnoreCase(keyName)
                        && keys.keyCode == keyCode
                        && keys.keyLocation == keyLocation)
        .findFirst()
  ...
  public static Keys get(KeyEvent keyEvent) {
    return get(
        KeyEvent.getKeyText(keyEvent.getExtendedKeyCode()),
        keyEvent.getKeyCode(),
