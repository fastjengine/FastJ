 ...
 *
 * <p>This class fires input events to {@link KeyboardActionListener}s or {@link
 * MouseActionListener}s in its lists of keyboard/mouse action listeners.
 */
public class InputManager {
  ...
  private static final Map<Integer, BiConsumer<MouseEvent, List<MouseActionListener>>>
      MouseActionProcessor =
          Map.of(
              MouseEvent.MOUSE_PRESSED,
                  (mouseEvent, mouseActionListenerList) -> {
  ...
  private static final Map<Integer, BiConsumer<KeyboardEvent, List<KeyboardActionListener>>>
      KeyboardActionProcessor =
          Map.of(
              KeyEvent.KEY_PRESSED,
                  (keyboardEvent, keyActionListenerList) -> {
                    for (KeyboardActionListener keyboardActionListener : keyActionListenerList) {
                      keyboardActionListener.onKeyRecentlyPressed(
   ...
   *
   * <p>This method does not allow for {@code KeyboardActionListener}s to be added more than once.
   *
  ...
  public void fireKeyEvent(KeyboardEvent keyboardEvent) {
    KeyboardActionProcessor.get(keyboardEvent.getKeyEvent().getID())
        .accept(keyboardEvent, keyActionListeners);
  }
   ...
   *
   * <p>This method does not allow for {@code MouseActionListener}s to be added more than once.
   *
   ...
   *
   * <p>If event processing is still going on when the event is received, the event gets added to
   * the backlog. That backlog gets emptied into the main event list after all the events in that
   * main list have been processed.
   *
   ...
   *
   * <p>This method also empties the event backlog into the main event set after all the current
   * events have been processed and removed.
   */
