...
public class Transform2D {
  ...
  public Pointf getTranslation() {
    return new Pointf(
        (float) translationTransform.getTranslateX(), (float) translationTransform.getTranslateY());
  }
  ...
  public float getRotation() {
    float approximatedRotation =
        (float)
            Math.toDegrees(
                Math.atan2(rotationTransform.getShearY(), rotationTransform.getScaleY()));
    ...
    throw new IllegalStateException(
        "Something went wrong calculating the rotation.... we expected "
            + rotation
            + ", but we got "
            + approximatedRotation
            + " instead. :(");
  }
