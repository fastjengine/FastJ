...
/**
 * Secondary mathematics class to provide useful utility methods dealing primarily with {@code
 * float}s.
 *
 * @author Andrew Dey
 * @since 1.0.0
...
public class Maths {
   ...
   *
   * <p>Unlike the other random generator methods, this method <b>includes</b> the max number as a
   * possibility.
   *
   * @param min The minimum number possible, inclusive.
   * @param max The maximum number possible, inclusive.
   * @return Randomized integer value within the range of the specified parameters, including both
   *     the min and max as possible values.
   */
   ...
   *
   * <p>If the two edges are equidistant from the {@code num}, then the right edge will be returned.
   *
  ...
  /**
   * Checks for 'equality' between two floating point values through ensuring their difference is
   * less than the defined float precision value {@link #FloatPrecision}.
   *
   ...
   * @return The linearly interpolated value. (If the interpolation value {@code t} is within a
   *     range of {@code 0.0} to {@code 1.0}, then this value will be within the range of the
   *     provided values {@code f} and {@code f1}).
   *     <p>This value can also be used to calculate {@code t} when used alongside {@code f} and
   *     {@code f1} in an inverse linear interpolation calculation {@link Maths#inverseLerp(float,
   *     float, float)}.
  ...
  /**
   * Calculates the linear interpolation value based on the provided {@code f} and {@code f1} range,
   * and the value {@code v}.
   *
   * @param f The starting value.
   * @param f1 The ending value.
   * @param v The {@code float} value representing the "result" of linear interpolation between the
   *     two values {@code f} and {@code f1}.
   * @return The value which, when used to calculate linear interpolation with the same {@code f}
   *     and {@code f1} values (see: {@link Maths#lerp(float, float, float)}), will get {@code v} as
   *     a result.
   * @author <a href="https://github.com/YeffyCodeGit" target="_blank">YeffyCodeGit</a>
   * @since 1.5.0
   */
  ...
  /**
   * Scales the provided number {@code num} on a scale of {@code 0.0} to {@code 1.0} based on the
   * provided {@code minimum} and {@code maximum} values.
   *
  ...
  public static float normalize(float num, float min, float max) {
    if (num > max) {
      throw new IllegalArgumentException(
          "The provided number must be less than the maximum number.");
    }
    if (num < min) {
      throw new IllegalArgumentException(
          "The provided number must be more than the minimum number.");
    }
  ...
  /**
   * Scales a normalized number {@code normalizedNum} on a scale of {@code min} to {@code max},
   * based on the provided {@code min} and {@code max} values.
   *
   ...
   * @return The denormalized version of {@code normalizedNum}, on a scale of {@code min} to {@code
   *     max}.
   * @since 1.5.0
   */
