...
public class Display {
   ...
   *
   * <p>Setting this resets the displayed title.
   *
   ...
   *
   * <p>This title takes from the original title, and should only be used for temporary changes of
   * the title.
   *
   ...
   *
   * <p>This is the resolution that the window sees, and is a scaled version of the game resolution.
   *
   ...
   *
   * <p>This is the resolution that the game itself is intended for, and looks the best on.
   *
   * @param res The new resolution to be set to.
   */
   ...
   *
   * <p>The scale is represented as an expression of {@code windowResolution / internalResolution}.
   *
   * <p>The values for the x and y of the returned {@code Pointf} are {@code 0 < x <= 1}.
   *
  ...
  public Graphics2D getGraphics() {
    return prepareGraphics(
        (Graphics2D) drawingCanvas.getBufferStrategy().getDrawGraphics(), Camera.Default);
  }
  ...
  public void resizeDisplay(Point newResolution) {
    ...
    outputDisplay
        .getContentPane()
        .setPreferredSize(new Dimension(newResolution.x, newResolution.y));
   ...
   */
  public void render(
      Map<String, GameObject> gameObjects, Map<String, UIElement> gui, Camera camera) {
    ...
    try {
      ...
      drawGraphics.clearRect(
          ...
          // add 1 to these values to account for floating point cutoff, since this has to be all
          // integers
      ...
      for (GameObject gameObject : gameObjects.values()) {
        try {
          ...
          FastJEngine.error(
              CrashMessages.RenderError.errorMessage + " | Origin: " + gameObject.getID(),
              exception);
          return;
        }
      }
      ...
      for (UIElement guiObj : gui.values()) {
        try {
          ...
          FastJEngine.error(
              CrashMessages.RenderError.errorMessage + " | Origin: " + guiObj.getID(), exception);
          return;
        }
      }
   ...
   *
   * <p>This takes advantage of the {@code RenderingHints} class, allowing the programmer to change
   ...
   *
   * @param renderSetting The {@link RenderSettings} value used to change the global rendering
   *     settings.
   */
   ...
   *
   * <p>This method should only be called once per instance of the {@code Display}. Furthermore,
   * this method should not be used on the default window for the game engine - it is initialized
   * internally.
   */
  public void init() {
    if (!isReady) {
  ...
  private void initDisplay() {
    ...
    outputDisplay
        .getContentPane()
        .setPreferredSize(new Dimension(windowResolution.x, windowResolution.y));
  ...
  /**
   * Removes the {@link #outputDisplay} from sight, allowing values like the title-bar to be
   * changed.
   */
   ...
   *
   * <p>This scales the object by the current display's resolution scale, sets its rendering hints
   * to the current {@code Display}'s rendering hints, and transforms it based on the specified
