...
/**
 * Class to streamline working with the {@link GradientPaint} class (and its implementations) within
 * FastJ.
 */
public class Gradients {

  /**
   * The maximum amount of colors allowed in a single {@link LinearGradientBuilder} or {@link
   * RadialGradientBuilder}.
   */
  public static final int MaximumColorCount = 8;

  /**
   * The minimum amount of colors allowed in a single {@link LinearGradientBuilder} or {@link
   * RadialGradientBuilder}.
   */
  public static final int MinimumColorCount = 2;

  /**
   * The minimum radius allowed in a single {@link RadialGradientBuilder}, including the value of
   * this constant.
   */
  ...
  public static LinearGradientBuilder linearGradient(
      Drawable drawable, Boundary start, Boundary end) {
   ...
   *
   * @param drawable The {@code Drawable} defining the centerpoint and radius to build the gradient
   *     with.
  ...
  /**
   * Generates a random linear gradient from the specified {@code drawable} and {@code start}/{@code
   * end} values, with random colors.
   *
   ...
   */
  public static LinearGradientPaint randomLinearGradient(
      Drawable drawable, Boundary start, Boundary end) {
  ...
  /**
   * Generates a random linear gradient from the specified {@code drawable} and {@code start}/{@code
   * end} values, with random colors including random alpha values.
   *
   * @param drawable The {@link Drawable} to calculate start/end {@code Pointf}s.
   * @param start The {@link Boundary} specifying the starting area for the gradient.
   * @param end The {@link Boundary} specifying the ending area for the gradient.
   * @return The randomly generated {@link LinearGradientPaint}.
   */
  public static LinearGradientPaint randomLinearGradientWithAlpha(
      Drawable drawable, Boundary start, Boundary end) {
    LinearGradientBuilder linearGradientBuilder = linearGradient(drawable, start, end);
  ...
  /**
   * Generates a random linear gradient from the specified {@code start}/{@code end} values, with
   * random colors.
   *
  ...
  /**
   * Generates a random radial gradient from the specified {@code center} and {@code radius} values,
   * with random colors.
   *
  ...
  /**
   * Generates a random radial gradient from the specified {@code center} and {@code radius} values,
   * with random colors including random alpha values.
   *
   * @param center The {@link Pointf} specifying the centerpoint for the gradient.
   * @param radius The {@code float} specifying the radius for the gradient.
   * @return The randomly generated {@link RadialGradientPaint}.
  ...
  static void colorLimitCheck(int count) {
    if (count >= MaximumColorCount) {
      throw new IllegalStateException(
          "Gradients cannot contain more than " + MaximumColorCount + " colors.");
    }
  }
    ...
    if (count < MinimumColorCount) {
      throw new IllegalStateException(
          "Gradients must contain at least 2 colors."
              ...
              + Arrays.toString(Arrays.stream(colors).filter(Objects::nonNull).toArray()));
    }
  }

  /**
   * Generates an array of linear intervals from {@code 0.0f} to {@code 1.0} inclusive, based on the
   * size provided.
   *
