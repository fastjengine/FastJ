...
public class RadialGradientBuilder implements GradientBuilder {
  ...
  private void position(Drawable drawable) {
    ...
    Pointf horizontalSide =
        Pointf.subtract(drawable.getBound(Boundary.TopRight), drawable.getBound(Boundary.TopLeft));
    Pointf verticalSide =
        Pointf.subtract(
            drawable.getBound(Boundary.BottomLeft), drawable.getBound(Boundary.TopLeft));

    Pointf smallestSide =
        horizontalSide.squareMagnitude() < verticalSide.squareMagnitude()
            ? horizontalSide
            : verticalSide;
  ...
  /**
   * Sets the centerpoint and radius for the builder, based on the provided {@link Pointf} and float
   * values.
   *
   ...
   *
   * <p>The amount of colors used in a {@link RadialGradientBuilder} not exceed {@link
   * Gradients#MaximumColorCount}.
   *
   ...
   *
   * <p>The amount of colors used in a {@link RadialGradientBuilder} cannot exceed {@link
   * Gradients#MaximumColorCount}.
   *
   * @param colors The {@code Color}s being added. This parameter must not cause the builder to
   *     increase over 8 colors.
  ...
  /**
   * Creates a new {@link RadialGradientPaint} object, using the data provided by other method
   * calls.
   *
  ...
  public RadialGradientPaint build() {
    ...
    return new RadialGradientPaint(
        center.x, center.y, radius, Arrays.copyOf(fractions, count), Arrays.copyOf(colors, count));
  }
