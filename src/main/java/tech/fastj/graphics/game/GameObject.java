 ...
 *
 * <p>The {@code GameObject} class is one of the most useful backing classes in the engine. It
 * contains the logic needed to transform objects that can be rendered to the screen -- polygons,
 * models, and (still a work in progress) text.
 *
 * @author Andrew Dey
 * @since 1.0.0
 */
public abstract class GameObject extends Drawable {
  ...
  /**
   * Calls the {@link Behavior#init(GameObject)} method for each of the {@code GameObject}'s
   * behaviors.
   */
  ...
  /**
   * Calls the {@link Behavior#update(GameObject)} method for each of the {@code GameObject}'s
   * behaviors.
   */
   ...
   *
   * <p>{@code Behavior}s can be added as many times as needed.
   *
   * @param behavior {@code Behavior} parameter to be added.
   * @param behaviorHandler Handler that the {@code GameObject} will be added to, as a behavior
   *     listener.
  ...
  /**
   * Destroys all references of the {@code GameObject}'s behaviors and removes its references from
   * the scene.
   *
   * @param origin {@code Scene} parameter that will have all references to this {@code GameObject}
   *     removed.
   */
  @Override
  ...
  public String toString() {
    return "GameObject{" + "behaviors=" + behaviors + ", transform=" + transform + '}';
  }
}
