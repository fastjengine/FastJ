...
public abstract class UIElement extends Drawable implements MouseActionListener {
  ...
  /**
   * Instantiates the {@code UIElement}'s internals, and adds it to the origin scene as a ui
   * element/mouse listener.
   *
  ...
  /**
   * Sets the "onAction" condition, the determinant of whether the ui elements' {@code onAction}
  ...
  /**
   * Renders the {@code UIElement} to the parameter {@code Graphics2D} object, aligning with the
   * window by rendering at the inverse translation of the specified {@code Camera}.
   *
  ...
  /**
   * Removes the {@code UIElement}'s references in the specified scene as a GUI object and as a
   * mouse listener.
   *
   * @param origin {@code Scene} parameter that will have all references to this {@code UIElement}
   *     removed.
   */
  @Override
  ...
  /**
   * Removes the {@code UIElement}'s references in the specified {@code SimpleManager} as a GUI
   * object and as a mouse listener.
   *
