 ...
 *
 * <p>A {@code Drawable} is any object that can be drawn to a {@code Display}, and destroyed (freed
 * from memory).
 *
 * @author Andrew Dey
 * @since 1.0.0
...
public abstract class Drawable extends TaggableEntity {
  ...
  /**
   * {@code boolean} representing the default value for if a {@code Drawable} should be rendered as
   * {@code true}.
   */
   ...
   *
   * <p>This also removes any internal references that the {@code Drawable} may have.
   *
   ...
   *
   * <p>Bounds are in the same order as specified in the {@link Boundary}.
   *
   * <p>If you're looking to get a specific bound, use {@code getBound(Boundary)} instead.
   *
   ...
   *
   * @param setTranslation {@code Pointf} parameter that the {@code Drawable}'s translation will be
   *     set to.
   * @return The {@code Drawable}, for method chaining.
   */
   ...
   *
   * @param scaleMod {@code Pointf} parameter that the {@code Drawable}'s width and height will be
   *     scaled by.
   ...
   * center.
   *
   * @param scaleXY float parameter that the {@code Drawable} will be scaled by, acting as both the
   *     x and y values.
   */
   ...
   * center.
   *
   * @param scaleMod {@code Pointf} parameter that the {@code Drawable} will be scaled by, based on
   *     its x and y values.
   */
  ...
  /**
   * Destroys the {@code Drawable}'s {@code Drawable} components, as well as any references the
   * {@code Drawable} has within the {@code SimpleManager} parameter.
   *
   * @param origin {@code SimpleManager} parameter that will have all references to this {@code
   *     Drawable} removed.
   */
  ...
  private void updateTransformedCollisionPath() {
    transformedCollisionPath =
        (Path2D.Float) collisionPath.createTransformedShape(transform.getAffineTransform());
  }
