...
public class ModelUtil {
  ...
  private static final Map<String, Function<List<String>, Polygon2D[]>> ModelParser =
      Map.of(SupportedModelFormats.Psdf, PsdfUtil::parse);

  private static final Map<String, BiConsumer<Path, Model2D>> ModelWriter =
      Map.of(SupportedModelFormats.Psdf, PsdfUtil::write);

  /**
   * Gets a {@code Polygon2D} array, loaded from a {@code .psdf} file.
   *
   * <p>This method allows the user to load an array of {@code Polygon2D}s from a single file,
   * decreasing the amount of models that have to be programmed in.
   *
   * <p>Furthermore, this allows for easy use of the {@code Model2D} class, allowing you to directly
  ...
  public static Polygon2D[] loadModel(Path fileLocation) {
    if (!Files.exists(fileLocation, LinkOption.NOFOLLOW_LINKS)) {
      throw new IllegalArgumentException(
          "A file was not found at \"" + fileLocation.toAbsolutePath() + "\".");
    }
    ...
    try {
      ...
      throw new IllegalStateException(
          CrashMessages.theGameCrashed(
              "an issue while trying to parse file \"" + fileLocation.toAbsolutePath() + "\"."),
          exception);
    }
  ...
  /**
   * Writes a model file containing the current state of the {@code Polygon2D}s that make up the
   * specified {@code Model2D}.
   *
  ...
  private static void checkFileExtension(String fileExtension) {
    if (Arrays.stream(SupportedModelFormats.values())
        .noneMatch(fileFormat -> fileFormat.equalsIgnoreCase(fileExtension))) {
      throw new IllegalArgumentException(
