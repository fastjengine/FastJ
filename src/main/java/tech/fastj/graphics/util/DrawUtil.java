 ...
 *
 * <p>This class is also used to load 2-dimensional model files, of the {@code ".psdf"} file
 * extension.
 *
 * @author Andrew Dey
 * @since 1.0.0
...
public final class DrawUtil {
   ...
   * array.
   *
   * <p><b>NOTE:</b> This method likely will not provide a completely accurate outline of the array
   * of {@code Polygon2D} objects.
   *
   * @param polygons The Array of {@code Polygon2D}s that will be used to create the outline of
   *     {@code Pointf}s.
  ...
  public static Pointf[] createCollisionOutline(Polygon2D[] polygons) {
    ...
    for (int i = (polygonsPoints.size() - 1); i > -1; i--) {
      ...
      // if a point intersects with more than one polygon, then it is an inner point and should be
      // removed
      for (Polygon2D polygon : polygons) {
        if (Path2D.Float.intersects(
            polygon.getCollisionPath().getPathIterator(null),
            polygonsPoints.get(i).x - 1f,
            polygonsPoints.get(i).y - 1f,
            2f,
            2f)) {
          intersectionCount++;
    ...
    System.arraycopy(
        unshiftedResult, shiftAmount, shiftedResult, 0, shiftedResult.length - shiftAmount);
    System.arraycopy(
        unshiftedResult, 0, shiftedResult, shiftedResult.length - shiftAmount, shiftAmount);
   ...
   *
   * <p>Order does not matter for equality checking.
   *
  ...
  public static boolean pathEquals(Path2D path1, Path2D path2) {
    ...
    if (pLength1 != pLength2) {
      throw new IllegalStateException(
          "Path lengths differ\nPath 1 had a length of "
              + pLength1
              + ", but path 2 had a length of "
              + pLength2
              + ".");
    }
   ...
   * @return Whether the two {@code Paint}s are equal.
   */
  private static boolean mGradientEquals(
      MultipleGradientPaint mGradientPaint1, MultipleGradientPaint mGradientPaint2) {
   ...
   * floats.
   *
   * <p>This creates an array with the following values:
   *
   * <pre>
   * new Pointf[] {
   *     new Pointf(x, y),
  ...
  /**
   * Creates a {@code Pointf} array of 4 points, based on the specified location {@code Pointf} and
   * size float.
   *
   * <p>This creates an array with the following values:
   *
  ...
  /**
   * Creates a {@code Pointf} array of 4 points, based on the specified x and y floats, and size
   * {@code Pointf}.
   *
   * <p>This creates an array with the following values:
   *
   ...
   *
   * <p><b>NOTE:</b> the {@code pts} parameter must have a length of 4.
   *
  ...
  public static Rectangle2D.Float createRectFromImage(BufferedImage source, Pointf translation) {
    return new Rectangle2D.Float(
        translation.x, translation.y, source.getWidth(), source.getHeight());
  }
  ...
  public static Color randomColor() {
    return new Color(
        (int) Maths.random(0, 255), (int) Maths.random(0, 255), (int) Maths.random(0, 255), 255);
  }
  ...
  /**
   * Generates a random {@link Font} using the available fonts on the system, with a font size
   * within the range of {@code 1-256}, inclusive.
   *
  ...
  /**
   * Creates a new {@link Color}, applying to it the linear interpolation of the two {@link Color}s
   * specified.
   *
   ...
   *     value will be used to linear interpolate all 4 of the {@code Color}'s values (red, blue,
   *     green, alpha).
   ...
   *     This value will be used to linear interpolate the {@code Color}'s red value.
   * @param t2 The second interpolation value to work with (preferably within a range of 0.0 to
   *     1.0). This value will be used to linear interpolate the {@code Color}'s green value.
   * @param t3 The third interpolation value to work with (preferably within a range of 0.0 to 1.0).
   *     This value will be used to linear interpolate the {@code Color}'s blue value.
   * @param t4 The fourth interpolation value to work with (preferably within a range of 0.0 to
   *     1.0). This value will be used to linear interpolate the {@code Color}'s alpha value.
   * @return A new {@code Color}, linearly interpolated as specified.
   * @see Maths#lerp(float, float, float)
   */
   ...
   *     Color}s. This value is used to calculate inverse linear interpolation of the colors' {@code
   *     red}, {@code green}, {@code blue}, and {@code alpha} values.
   * @return An array of floats containing the resulting inverse linear interpolations of the
   *     colors' {@code red}, {@code green}, {@code blue}, and {@code alpha} values.
   ...
   *     {@code Color}s. This value is used to calculate inverse linear interpolation of the colors'
   *     {@code red} value.
   * @param v2 The second value representing the "result" of linear interpolation between the two
   *     {@code Color}s. This value is used to calculate inverse linear interpolation of the colors'
   *     {@code green} value.
   * @param v3 The third value representing the "result" of linear interpolation between the two
   *     {@code Color}s. This value is used to calculate inverse linear interpolation of the colors'
   *     {@code blue} value.
   * @param v4 The fourth value representing the "result" of linear interpolation between the two
   *     {@code Color}s. This value is used to calculate inverse linear interpolation of the colors'
   *     {@code alpha} value.
   * @return An array of floats containing the resulting inverse linear interpolations of the
   *     colors' {@code red}, {@code green}, {@code blue}, and {@code alpha} values.
   * @see Maths#lerp(float, float, float)
   */
  public static float[] inverseColorLerp(
      Color c, Color c1, float v1, float v2, float v3, float v4) {
    return new float[] {
