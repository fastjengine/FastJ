...
public class MemoryAudioPlayer {
  ...
  static void playAudio(MemoryAudio audio) {
    ...
    if (clip.isOpen()) {
      FastJEngine.warning(
          "Tried to play audio file \""
              + audio.getAudioPath().toString()
              + "\", but it was already open (and likely being used elsewhere.)");
      return;
    }
    ...
    try {
      ...
      FastJEngine.error(
          CrashMessages.theGameCrashed("an error while trying to play sound."), exception);
    }
  }
  ...
  static void pauseAudio(MemoryAudio audio) {
    ...
    if (!clip.isOpen()) {
      FastJEngine.warning(
          "Tried to pause audio file \""
              + audio.getAudioPath().toString()
              + "\", but it wasn't being played.");
      return;
    }
  ...
  static void seekInAudio(MemoryAudio audio, long timeChange) {
    ...
    if (clip.isActive()) {
      FastJEngine.warning(
          "Tried to change the playback position of audio file \""
              + audio.getAudioPath().toString()
              + "\", but it was still running.");
      return;
    }

    long timeChangeInMilliseconds =
        TimeUnit.MICROSECONDS.convert(timeChange, TimeUnit.MILLISECONDS);
  ...
  static void setAudioPlaybackPosition(MemoryAudio audio, long playbackPosition) {
    ...
    if (clip.isActive()) {
      FastJEngine.warning(
          "Tried to set the playback position of audio file \""
              + audio.getAudioPath().toString()
              + "\", but it was still running.");
      return;
    }

    long playbackPositionInMilliseconds =
        TimeUnit.MICROSECONDS.convert(playbackPosition, TimeUnit.MILLISECONDS);
