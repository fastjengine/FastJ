...
public class MemoryAudioPlayer {
    ...
    static void playAudio(MemoryAudio audio, long playbackPosition) {
        ...
        if (clip.isOpen()) {
            Log.warn(MemoryAudioPlayer.class, "Tried to play audio file \"{}\", but it was already open (and likely being used elsewhere.)", audio.getAudioPath()
                .toString());
            return;
        }
        ...
        try {
            ...
            throw new IllegalStateException("IO read error while trying to open \"" + audio.getAudioPath()
    ...
    static void pauseAudio(MemoryAudio audio) {
        ...
        if (!clip.isOpen()) {
            Log.warn(MemoryAudioPlayer.class, "Tried to pause audio file \"{}\", but it wasn't being played.", audio.getAudioPath()
                .toString());
            return;
        }
    ...
    static void resumeAudio(MemoryAudio audio, long playbackPosition) {
        ...
        if (!clip.isOpen()) {
            Log.warn(MemoryAudioPlayer.class, "Tried to resume audio file \"{}\", but it wasn't being played.", audio.getAudioPath()
                .toString());
            return;
        }
    ...
    static long stopAudio(MemoryAudio audio) {
        ...
        if (!clip.isOpen()) {
            Log.warn(MemoryAudioPlayer.class, "Tried to stop audio file \"{}\", but it wasn't being played.", audio.getAudioPath()
                .toString());
    ...
    static void seekInAudio(MemoryAudio audio, long timeChange) {
        ...
        if (clip.isActive()) {
            Log.warn(MemoryAudioPlayer.class, "Tried to change the playback position of audio file \"{}\", but it was still running.", audio.getAudioPath()
                .toString());
            return;
        }
    ...
    static void setAudioPlaybackPosition(MemoryAudio audio, long playbackPosition) {
        ...
        if (clip.isActive()) {
            Log.warn(MemoryAudioPlayer.class, "Tried to set the playback position of audio file \"{}\", but it was still running.", audio.getAudioPath()
                .toString());
            return;
        }
    ...
    static void rewindAudioToBeginning(MemoryAudio audio) {
        ...
        if (clip.isActive()) {
            Log.warn(MemoryAudioPlayer.class, "Tried to rewind audio file \"{}\", but it was still running.", audio.getAudioPath()
                .toString());
            return;
        }
