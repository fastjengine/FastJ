 ...
 *
 * <p>This type of {@link Audio} is streamed from the file, reducing the amount of memory taken when
 * loading it.
 *
 * <p>In addition to {@link Audio the controls all Audio types in FastJ support}, {@code
 * StreamedAudio} supports the following extra controls:
 *
 * <ul>
 *   <li>Gain Controls
 *   <li>Pan Controls
...
public class StreamedAudio implements Audio {
  ...
  private void initializeStreamData() {
    sourceDataLine =
        Objects.requireNonNull(AudioManager.newSourceDataLine(audioInputStream.getFormat()));

    try {
      sourceDataLine.open(audioInputStream.getFormat());
    } catch (LineUnavailableException exception) {
      FastJEngine.error(
          CrashMessages.theGameCrashed("an error while trying to open sound."), exception);
    }
