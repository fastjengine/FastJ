 ...
 *
 * <p>This type of {@link Audio} requires that the audio is loaded into memory before accessing.
 *
 * <p>In addition to {@link Audio the controls all Audio types in FastJ support}, {@code
 * MemoryAudio} supports the following extra controls:
 *
 * <ul>
 *   <li>Explicit Looping Controls ({@link #setLoopCount(int) Loop count}, {@link
 *       #setLoopPoints(float, float) Loop points}, {@link #stopLoopingNow() immediate loop
 *       stopping})
 *   <li>Explicit Playback Position Controls ({@link #seek(long) Seeking}, {@link
 *       #rewindToBeginning() Rewinding})
 * </ul>
 *
 * @author Andrew Dey
 * @since 1.5.0
...
public class MemoryAudio implements Audio {
   ...
   *
   * <p>Made for use with {@link MemoryAudio#setLoopCount(int)}.
   */
   ...
   *
   * <p>Made for use with {@link MemoryAudio#setLoopPoints(float, float)}, as the first parameter.
   */
   ...
   *
   * <p>Made for use with {@link MemoryAudio#setLoopPoints(float, float)}, as the second parameter.
   */
   ...
   *
   * <ul>
   *   <li>This can be affected by {@link MemoryAudio#setShouldLoop(boolean)}, {@link
   *       MemoryAudio#setLoopPoints(float, float)}, and {@link MemoryAudio#setLoopCount(int)} (in
   *       most circumstances).
   * </ul>
   *
   ...
   *
   * <ul>
   *   <li>If the loop count (check using {@link MemoryAudio#getLoopCount()}) is set to {@code 0} or
   *       {@link MemoryAudio#StopLooping}, calling this method will have no effect on whether the
   *       audio will loop.
   * </ul>
   *
   ...
   *
   * <ul>
   *   <li>Both {@code loopStart} and {@code loopEnd} should be on a scale of {@code 0.0} to {@code
   *       1.0}.
   *   <li>Calling this method automatically tells the audio to loop (you can check this using
   *       {@link MemoryAudio#shouldLoop()}). See {@link MemoryAudio#setLoopCount(int)} and {@link
   *       MemoryAudio#setShouldLoop(boolean)} for ways to disable looping.
   * </ul>
   *
   * @param loopStart The point to loop back to; the starting point for the audio loop. This value
   *     is on a scale of {@code 0.0} to {@code 1.0}.
   * @param loopEnd The point at which to loop; the ending point for the audio loop. This value is
   *     on a scale of {@code 0.0} to {@code 1.0}.
   */
   ...
   *
   * <ul>
   *   <li>If you want to loop continuously, call this method with {@link
   *       MemoryAudio#ContinuousLoop} as the {@code loopCount} parameter.
   *   <li>If you want to disable looping, call this method with {@link MemoryAudio#StopLooping} as
   *       the {@code loopCount} parameter.
   * </ul>
   *
   ...
   *
   * <ul>
   *   <li>If the audio was set to loop (check using {@link MemoryAudio#shouldLoop()}) then calling
   *       this method will cause it to loop.
   *   <li>To stop the audio's playback, use {@link MemoryAudio#stop()}.
   ...
   * <ul>
   *   <li>If the audio was set to loop (check using {@link MemoryAudio#shouldLoop()}) then calling
   *       this method will cause it to continue looping.
   ...
   *
   * <ul>
   *   <li>To start the audio's playback again, use {@link MemoryAudio#play()}.
