...
public abstract class SceneManager implements LogicManager {
  ...
  public void receivedInputEvent(InputEvent inputEvent) {
    safeUpdate(
        FastJEngine.getDisplay(), () -> currentScene.inputManager.receivedInputEvent(inputEvent));
  }
   ...
   *
   * @param <T> The type of the scene being retrieved. This type must match the actual type of the
   *     retrieved scene, and must always extend {@link Scene}.
   ...
   *
   * <p>Instead of using this method to switch scenes, it is preferred that you use the {@code
   ...
   *
   * <p>This is the preferred method of switching from one scene to another. However, it does not
    ...
    if (!scenes.containsKey(nextSceneName)) {
      FastJEngine.error(
          ...
          new IllegalArgumentException(
              "A scene with the name: \"" + nextSceneName + "\" hasn't been added!"));
    }
  ...
  private boolean[] createSnapshot(Display display) {
    return new boolean[] {switchingScenes, display.isSwitchingScreenState()};
  }

  /**
   * Checks if the logic manager was switching scenes.
   *
   * <p>This method takes a boolean parameter, which is a record of whether the logic manager was
   * switching scenes at the beginning of the parent method call.
   *
   * <p>If the logic manager was not switching scenes, this method will error out the game engine.
   *
  ...
  /**
   * Safely initializes the current scene with the specified action, by first checking for null
   * pointers.
   *
   * @param display The {@code Display} that the game renders to.
  ...
  /**
   * Safely updates the current scene with the specified action, by first checking for null pointers
   * and scene initialization.
   *
   * @param display The {@code Display} that the game renders to.
   * @param action The action to safely run.
   */
   ...
   *
   * <p>If the current scene is null, this throws a NullPointerException that has a customized
   ...
   *
   * <p>If the current scene isn't initialized, this throws a NullPointerException with a message
   * that says so.
   */
   ...
   *
   * <p>If the scene name is already in use, this method will error out the game engine.
   *
   ...
   *
   * <p>If the scene name doesn't correspond with any scenes in the logic manager, this method will
  ...
  private void sceneExistenceCheck(String sceneName) {
    if (!scenes.containsKey(sceneName)) {
      FastJEngine.error(
          CrashMessages.SceneError.errorMessage,
          new IllegalArgumentException(
              "A scene with the name: \"" + sceneName + "\" hasn't been added!"));
    }
  }
}
