 ...
 *
 * <p>A {@code TaggableEntity} is an entity that can be given tags to describe what it is, such as
 * {@code "enemy"}, or {@code "player"}.
 *
 * <p>Each {@code TaggableEntity} can be added to a list of taggable entities for each {@code
 * Scene}. From the scene, you can find all entities with a certain tag, as well as finding the
 * first entity with a tag.
 *
 * <p>This can be helpful for a few reasons:
 *
 * <ul>
 *   <li>Finding a wide range of objects with a specified tag.
 *   <li>Finding the first object with a specified tag.
 * </ul>
 *
 * @author Andrew Dey
 * @since 1.0.0
...
public abstract class TaggableEntity {
   ...
   *
   * @param <T> The return type, which must extend {@code TaggableEntity}. By default, the return
   *     type is {@code TaggableEntity}.
   ...
   *
   * <p>This can also remove this entity from the scene's list of tagged entities if the entity has
   * no more tags left.
   *
   * @param <T> The return type, which must extend {@code TaggableEntity}. By default, the return
   *     type is {@code TaggableEntity}.
   * @param tag Tag to be removed from the object;s list of tags.
   * @param originScene Scene which this object, if it no longer has any tags, will be removed from
   *     as a {@code TaggableEntity}.
   * @return This instance of the {@code TaggableEntity} (or the specified type), for method
   *     chaining.
   */
  @SuppressWarnings("unchecked")
