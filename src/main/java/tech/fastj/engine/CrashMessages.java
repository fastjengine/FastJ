...
public enum CrashMessages {
  ...
  /**
   * "The game crashed, due to a method being called that can only be called before the program
   * starts."
   */
  CalledAfterRunError(
      theGameCrashed("a method being called that can only be called before the program starts.")),
  ...
  /**
   * Creates a string depicting an error caused by an illegal action in the method of the specified
   * class.
   *
   * <p>Example:
   *
  ...
  /**
   * Creates a string starting with {@code "The game crashed, due to "}, adding the specified error
   * message to the end.
   *
